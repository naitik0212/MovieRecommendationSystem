...
def inputgen():
    ...
    return data_x,data_y,n_users,n_movies
...
def train_model(model,train_x,train_y,model_number = 0,learn_rate=0.01):
    ...
    model.fit(train_x, train_y, batch_size=64, epochs=20,validation_split=0.2, callbacks=callbacks_list, verbose=1)
...
train_x,train_y,m,n = inputgen()
train_x,test_x,train_y,test_y = split_data(train_x,train_y)
model = construct_model(train_x,train_y,m,n)
